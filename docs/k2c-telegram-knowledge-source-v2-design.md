...
- 状态：**FROZEN**（2026-09-18 本机审阅通过后冻结）
- 日期：2026-09-18
...
- 冻结依据：`docs/k2c-telegram-knowledge-source-v2-本机审阅意见.md`（2026-09-18，结论：0 处与本机实现冲突；3 处清单补全/澄清 + 3 处编辑性修正已并入本稿并在下方修订记录留痕）
- 冻结效力：本稿为 Telegram Source V2 正式实施方案的唯一设计输入；后续修订须走显式版本变更，不得原地改写冻结正文

---

# V1 → V2 修订记录

1. **§15 重写**：`target=k2c` 事实更新——已于 2026-09-15 合并、2026-09-18 随 rebase+push 同步 origin/main（`4ca311e`，local==origin），从"系统级前置阻塞"改为"回归契约"。
2. **§17 扩展为 Notification Runtime**：NotificationPort / WxPusherAdapter、`event_id` 发送幂等、delivery log、§17.5 双 LaunchAgent 调度（watch=KeepAlive，digest=StartCalendarInterval）与持久 cursor 补算。
3. **§8.4 新增 SQLite 并发契约**：WAL + foreign_keys + busy_timeout + watcher 单写队列 + maintenance lock。
4. **§13.5 新增 Source Handoff v2 迁移影响面**；冻结稿按本机审阅补全两处遗漏（`templates/job.yaml`、`schemas/source-handoff.example.json`）并澄清"Source Router"实指 `cli route` 文件路由阶段（provider 为动态引用）。
5. **§10.4 + §21.1 新增**：规则优先的模型策略；Source AI Budget 独立记账，复用现有 `budget.py` 的 acquire/outcome/quota/breaker 设计原则（不复用 Target 业务账本）。
6. **§11.2 / §11.3 兴趣清单对齐本机画像**：INCLUDE 增补民宿/短租/OTA/本地生活、自媒体/内容运营/抖音/直播电商/带货、企业分析/投资纪律/长期投资方法/风险管理；EXCLUDE 精确到个股/短线/行情预测，并加入"投资认知类内容优先 INCLUDE/REVIEW"的正例保护。
7. **§3.4 可选依赖策略** + **§7.3 首次认证为显式运维步骤**（本机无任何可复用 Telegram session，已核实）。
8. **§5.2 补 `processing.require_orico` 死配置警告**：`config.py:31` 仅有定义，src 运行时零读取（已核实），实际阻断靠 doctor `orico_mounted`；Telegram watcher 必须主动探测 ORICO。
9. **编辑性修正（本机审阅 2026-09-18）**：§24.2 PDF 流程的 Noise 措辞与 §10.4 对齐（规则级预筛、不调模型）；§26.H 补"错过的 launchd 时刻按 §17.5 持久 cursor 补算"；§8.2 `downloads` 表补主键。
...
```text
ProviderName / Pydantic Literal（models.py）
JobRequest.provider 字段
CLI provider choices
source register validation（含 provider 条件分支，如 baidu /apps/bdpan 范围门）
Source Handoff schema（schemas/source-handoff.example.json）
unknown-key / schema-version 行为（完成门当前 schema_version != 1 即拒）
doctor provider mapping（CLOUD_SKILL_KEYS）
fixtures（tests/fixtures/*-source-handoff.json）
templates/job.yaml（provider 注释枚举）
unit / contract tests
report rendering
Source Router（实为 cli route 文件路由阶段；provider 为动态引用，仅需补 telegram 用例）
```
...
- REVIEW 即时通知；
- 不实时推送普通 Telegram 原文；
- Digest 覆盖 08:00 / 12:00 / 20:00 三个窗口；错过的 launchd 时刻按 §17.5 持久 cursor 补算，不算数据丢失。
