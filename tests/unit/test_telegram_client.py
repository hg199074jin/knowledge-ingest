...
# ---------- adapter 映射（telethon 已安装时执行；CI 自动跳过） ----------

telethon = pytest.importorskip("telethon")


def test_adapter_lazy_import_available():
    from knowledge_ingest.telegram.client_port import (
        TelegramDependencyMissingError as dep,
    )
    ...
    assert dep is TelegramDependencyMissingError


def test_adapter_flood_wait_mapping(tmp_path):
...
def test_adapter_rpc_error_mapping(tmp_path):
...
def test_adapter_to_event_mapping():
...
def test_adapter_to_event_with_document():
    from knowledge_ingest.telegram.telethon_adapter import _to_event
